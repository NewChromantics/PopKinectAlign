...
namespace freenect
{
	...
	{		
		...
		/// </summary>
		public void Open(bool CameraEnabled=true,bool MotorEnabled=true,bool AudioEnabled=true)
		{
			int SubDeviceEnum = 0;
			SubDeviceEnum |= AudioEnabled ? (int)freenect.SubDevice.Audio : 0;
			SubDeviceEnum |= CameraEnabled ? (int)freenect.SubDevice.Camera : 0;
			SubDeviceEnum |= MotorEnabled ? (int)freenect.SubDevice.Motor : 0;
			KinectNative.freenect_select_subdevices( KinectNative.Context,SubDeviceEnum);
