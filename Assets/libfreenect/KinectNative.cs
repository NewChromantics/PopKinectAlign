...
namespace freenect
{
	...
	class KinectNative
	{
		...
		//	gr
		[DllImport("freenect", CallingConvention=CallingConvention.Cdecl)]
		public static extern int freenect_supported_subdevices();

		[DllImport("freenect", CallingConvention=CallingConvention.Cdecl)]
		public static extern int freenect_enabled_subdevices(IntPtr context);

		//	gr: won't start (-2 upload_firmware failed) when starting with audio on OSX (for me at least). So added the ability to select functionality without
		[DllImport("freenect", CallingConvention=CallingConvention.Cdecl)]
		public static extern void freenect_select_subdevices(IntPtr context,int freenect_device_flags);
